...
const envSchema = z.object({
    ...
    ENABLE_HTTP: z.coerce.boolean().default(true),
    ENABLE_TERMINAL: z.coerce.boolean().default(true),
...
export function createConfig(env: NodeJS.ProcessEnv): AppConfig {
    ...
    const trelloApiKey = parsed.TRELLO_API_KEY?.trim();
    const trelloToken = parsed.TRELLO_TOKEN?.trim();
    ...
    return {
        ...
        integrations: {
            trello: {
                ...(trelloApiKey ? { apiKey: trelloApiKey } : {}),
                ...(trelloToken ? { token: trelloToken } : {}),
            },
        },
        persistence: {
