...
test("createConfig applies sensible defaults", () => {
    ...
    assert.equal(config.memory.backend, "local");
    assert.equal(config.http.allowedOrigin, undefined);
    ...
    assert.equal(config.providers.zep.baseUrl, "https://api.getzep.com");
});
